...
def _max_heapify_up(array: Array[Item], index: int) -> None:
    ...
    if array.get_at(index) > array.get_at(parent_index):
        _swap(array, parent_index, index)
...
def _max_heapify_down(array: Array[Item], index: int, length: int) -> None:
    ...
    """
    left_index = _get_left(index)
    if left_index >= length:
        return
    right_index = _get_right(index)
    if right_index >= length or array.get_at(left_index) > array.get_at(right_index):
        child_index = left_index
    else:
        child_index = right_index
    if array.get_at(child_index) > array.get_at(index):
        _swap(array, child_index, index)
        _max_heapify_down(array, child_index, length)

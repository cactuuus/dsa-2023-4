...
Sorted List Priority Queues Solution
"""
...
class SortedListPriorityQueue(Base[Priority, Item]):
    ...
    def enqueue(self, priority: Priority, item: Item) -> None:
        ...
        for index in reversed(range(self.get_length() - 1)):
            item_priority, _ = self._items.get_at(index)
            if self._is_higher_priority(priority, item_priority):
                return

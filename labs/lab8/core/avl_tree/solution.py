...
class _AVLSubtree(Base[Key, Value]):
    ...
    def rotate_cw(self) -> None:
        ...
        if self.is_left_empty():
            raise ValueError
        old_left = self._left
        self._left = old_left._right
        if self.has_left():
            self._left._parent = self
        old_left._right = self
        if self.is_root():
            self._tree._root = old_left
        elif self.is_left():
            self._parent._left = old_left
        else:
            self._parent._right = old_left
        old_left._parent = self._parent
        self._parent = old_left
        self.recalculate()
        old_left.recalculate()
    ...
    def rotate_acw(self) -> None:
        ...
        self._right = old_right._left
        if self.has_right():
            self._right._parent = self
        old_right._left = self
        if self.is_root():
            self._tree._root = old_right
        elif self.is_right():
            self._parent._right = old_right
        else:
            self._parent._left = old_right
        old_right._parent = self._parent
        self._parent = old_right
        self.recalculate()
        old_right.recalculate()
    ...
    def rebalance(self) -> bool:
        ...
        if self._balance_factor == -2:
            if self._left._balance_factor == 1:
                self._left.rotate_acw()
            self.rotate_cw()
            return True
        if self._balance_factor == 2:
            if self._right._balance_factor == -1:
                self._right.rotate_cw()
            self.rotate_acw()
            return True
        return False
